...
public partial class FormGridPrestacaoServicos : BaseGridForm
{
    ...
    public FormGridPrestacaoServicos()
    ...
    {
        base.montaTela();
        this.Title += "Prestação de Serviços";
        subTitulo.Text = "Prestação de Serviços";
        botaoNovo.NavigateUrl = "FormEditCadPrestacaoServicos.aspx";

        dsDados.Tables.Add(tbPrestacaoServicos);
        repeaterDados.DataSource = dsDados;
        repeaterDados.DataMember = dsDados.Tables["tbPrestacaoServicos"].TableName;
        repeaterDados.ItemDataBound += new RepeaterItemEventHandler(repeaterDados_ItemDataBound);

        if (!Page.IsPostBack)
        {
            comboOrdenar.Items.Add(new ListItem("Nome", "CPS.NOME"));
            comboOrdenar.Items.Add(new ListItem("Descrição", "CPS.DESCRICAO"));
            comboOrdenar.Items.Add(new ListItem("Emitente", "dbo.Emitentes_Prestacao_Servicos(CPS.COD_PRESTACAO_SERVICO)"));
    ...
    protected override void montaGrid()
    {
        ...
        totalRegistros = prestacao_servico.totalRegistros(fNome, fDescricao, fEmitente);
        tbPrestacaoServicos.Clear();
        prestacao_servico.listaPaginada(ref tbPrestacaoServicos, fNome, fDescricao, fEmitente, paginaAtual, ordenacao);
    ...
    protected override void repeaterDados_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        if (e.Item.ItemType != ListItemType.Separator)
        {
            ...
            linkAlterar.NavigateUrl = "FormEditCadPrestacaoServicos.aspx?id=" + check.Value;
        }
    }
            ...
            foreach (RepeaterItem item in repeaterDados.Items)
            {
                if (item.ItemType != ListItemType.Separator)
                {
                    ...
                    if (check.Checked)
                    {
                        try
                        {
                            prestacao_servico.cod_prestacao_servico = Convert.ToInt32(check.Value);
                            prestacao_servico.nome = nome.Text.Replace("'", " ");
                            erros.AddRange(prestacao_servico.deletar());
                        }
                        catch
                        {
                            erros.Add("Prestação de Serviço " + prestacao_servico.nome + ": Não foi possivel excluir, pois o mesmo está sendo utilizado.");
                        }
                    }
                }
            }
